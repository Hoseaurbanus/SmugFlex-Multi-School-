import{r as h,j as e}from"./radix-CQCJLaWg.js";import{u as O,q as ne,B as A}from"./index-RFrZialg.js";import{N as re,ap as ie,a0 as ae}from"./utils-CNihwnzq.js";const se=`
@media print {
  body * {
    visibility: hidden;
  }
...
`;function de({student:w,studentClass:N,result:t,detailedScores:u,showActions:v=!1,onDownload:z,onPrint:C,onApprovePrint:M,currentUser:_}){const{schoolSettings:s,loadSchoolSettings:R,students:T,classes:W,teachers:P,scores:p,subjectAssignments:g,subjects:j,affectiveDomains:f,psychomotorDomains:L,loadScoresFromAPI:U,loadSubjectAssignmentsFromAPI:G,loadSubjectsFromAPI:$,loadAffectiveDomainsFromAPI:K,loadPsychomotorDomainsFromAPI:V,getClassTeacher:X,loadClassesFromAPI:H}=O(),[le,ce]=h.useState(!1),[I,E]=h.useState([]);h.useEffect(()=>{s?.resumption_date||R()},[s?.resumption_date,R]),h.useEffect(()=>{const o=document.createElement("style");return o.textContent=se,document.head.appendChild(o),()=>{document.head.removeChild(o)}},[]);const q=()=>{window.print()};h.useEffect(()=>{t&&t.student_id&&(Z(),Y())},[t]),h.useEffect(()=>{W.length===0&&H().catch(()=>{})},[W.length,H]);const Y=async()=>{if(!(!t||!t.student_id))try{await Promise.all([f.length===0&&K(),L.length===0&&V()])}catch(o){console.error("Error loading domain data:",o)}},J=()=>!t||!t.student_id?{}:f.find(n=>n.student_id===t.student_id&&n.academic_year===t.academic_year&&n.term===t.term)||{},Q=()=>!t||!t.student_id?{}:L.find(n=>n.student_id===t.student_id&&n.academic_year===t.academic_year&&n.term===t.term)||{},Z=async()=>{if(!(!t||!t.student_id))try{await Promise.all([p.length===0&&U(),g.length===0&&G(),j.length===0&&$()]);let o=p.filter(n=>n.student_id===t.student_id&&n.academic_year===t.academic_year&&n.term===t.term);if(o=o.map(n=>{const a=g.find(d=>d.id===n.subject_assignment_id),l=a?j.find(d=>d.id===a.subject_id):null,S=a?P.find(d=>d.id===a.teacher_id):null,y=p.filter(d=>{const r=g.find(k=>k.id===d.subject_assignment_id);return r&&r.subject_id===a?.subject_id&&d.academic_year===t.academic_year&&d.term===t.term&&d.total>0}).map(d=>d.total||0),b=y.length>0?y.reduce((d,r)=>d+r,0)/y.length:0,D=y.length>0?Math.min(...y):0,B=y.length>0?Math.max(...y):0;return{...n,subject_name:l?l.name:n.subject_name||"Unknown Subject",subject_teacher:S?`${S.firstName} ${S.lastName}`:"Not Assigned",class_average:parseFloat(b.toFixed(2)),class_minimum:D,class_maximum:B}}).sort((n,a)=>n.subject_name.localeCompare(a.subject_name)),u&&u.length>0)E(u);else if(t.scores&&t.scores.length>0){const n=t.scores.map(a=>{const l=g.find(r=>r.id===a.subject_assignment_id),S=l?j.find(r=>r.id===l.subject_id):null,F=l?P.find(r=>r.id===l.teacher_id):null,b=p.filter(r=>{const k=g.find(oe=>oe.id===r.subject_assignment_id);return k&&k.subject_id===l?.subject_id&&r.academic_year===t.academic_year&&r.term===t.term&&r.total>0}).map(r=>r.total||0),D=b.length>0?b.reduce((r,k)=>r+k,0)/b.length:0,B=b.length>0?Math.min(...b):0,d=b.length>0?Math.max(...b):0;return{...a,subject_name:S?S.name:a.subject_name||"Unknown Subject",subject_teacher:F?`${F.firstName} ${F.lastName}`:"Not Assigned",class_average:parseFloat(D.toFixed(2)),class_minimum:B,class_maximum:d}}).sort((a,l)=>a.subject_name.localeCompare(l.subject_name));E(n)}else E(o)}catch(o){console.error("Error loading detailed scores:",o),E([])}},x=w||T.find(o=>o.id===t.student_id),i=N||W.find(o=>o.id===t.class_id),ee=()=>{if(t?.class_teacher_name&&t.class_teacher_name.trim()!=="")return t.class_teacher_name;if(i?.classTeacher)return i.classTeacher;if(i?.classTeacherId){const o=P.find(n=>n.id===i.classTeacherId);if(o)return`${o.firstName} ${o.lastName}`}if(i?.id){const o=X(i.id);if(o)return`${o.firstName} ${o.lastName}`}return"_________________"},te=o=>o>=80?{grade:"A",remark:"Excellent"}:o>=70?{grade:"B",remark:"Very Good"}:o>=60?{grade:"C",remark:"Good"}:o>=50?{grade:"D",remark:"Satisfactory"}:o>=45?{grade:"E",remark:"Fair"}:{grade:"F",remark:"Fail"},c=o=>o===5?"Excellent":o===4?"Very Good":o===3?"Good":o===2?"Fair":"Poor",m=o=>{const n={attentiveness:"Attentiveness",honesty:"Honesty",neatness:"Neatness",obedience:"Obedience",sense_of_responsibility:"Sense of Responsibility"},a={attention_to_direction:"Attention to Direction",considerate_of_others:"Considerate of Others",handwriting:"Handwriting",sports:"Sports",verbal_fluency:"Verbal Fluency",works_well_independently:"Works Well Independently"};return n[o]||a[o]||o.replace(/_/g," ").replace(/(?:^|\s)\S/g,l=>l.toUpperCase())};return _?.role==="admin"||t.print_approved,e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        @media print {
          @page {
            size: A4;
            margin: 10mm;
      ...
      `}),e.jsxs("div",{className:"min-h-screen bg-gray-100",children:[e.jsx("div",{className:"no-print bg-white shadow-md border-b border-gray-200 sticky top-0 z-50",children:e.jsx("div",{className:"max-w-7xl mx-auto px-4 sm:px-6 lg:px-8",children:e.jsxs("div",{className:"flex justify-between items-center py-4",children:[e.jsxs("div",{className:"flex items-center gap-4",children:[e.jsxs(A,{variant:"outline",onClick:t,className:"flex items-center gap-2",children:[e.jsx(re,{className:"w-4 h-4"}),"Back to Results"]}),e.jsxs("div",{children:[e.jsxs("h1",{className:"text-xl font-semibold text-gray-900",children:[C.firstName," ",C.lastName," - Result Sheet"]}),e.jsxs("p",{className:"text-sm text-gray-600",children:[R?.name," • ",_.term," • ",_.academic_year]})]})]}),e.jsxs("div",{className:"flex items-center gap-3",children:[e.jsxs(A,{variant:"outline",onClick:W,className:"flex items-center gap-2",children:[e.jsx(ie,{className:"w-4 h-4"}),"Print"]}),e.jsxs(A,{variant:"outline",onClick:P,className:"flex items-center gap-2","data-download-pdf":!0,children:[e.jsx(ae,{className:"w-4 h-4"}),"Download PDF"]})]})]})})}),e.jsx("div",{className:"full-page-container py-8",children:e.jsxs("div",{className:"max-w-7xl mx-auto px-4 sm:px-6 lg:px-8",children:[e.jsx("div",{className:"bg-white shadow-2xl mx-auto overflow-auto",style:{width:"210mm",minHeight:"297mm",maxWidth:"100%",aspectRatio:"210/297",overflow:"visible"},children:e.jsx("div",{className:"transform-gpu",style:{transform:"scale(1)",transformOrigin:"top center",width:"100%",height:"100%",overflow:"visible"},children:e.jsx(de,{student:C,studentClass:R,result:_,showActions:!1,currentUser:{role:"admin"}})})}),e.jsxs("div",{className:"no-print mt-8 text-center text-gray-600",children:[e.jsx("p",{className:"text-sm",children:"This is displayed in A4 format. Use the Print button to print or Download PDF to save."}),e.jsx("p",{className:"text-xs mt-2",children:"The result sheet is optimized for A4 paper size (210mm × 297mm)."})]})]})})]})]})}export{he as F,de as S};
