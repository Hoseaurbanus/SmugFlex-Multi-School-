...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPrimitive.Portal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={`fixed z-50 gap-4 bg-white p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out ${
        side === 'right'
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title ref={ref} className={`text-lg font-semibold text-[#1F2937] ${className || ''}`} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description ref={ref} className={`text-sm text-[#6B7280] ${className || ''}`} {...props} />
));
